...
interface AppWithInternals {
  ...
  internalPlugins: {
    getPluginById(
      id: string
    ): { instance?: { options?: Record<string, unknown> } } | undefined;
  };
  foldManager: {
...
function getDailyNoteSettings(): IPeriodicNoteSettings {
  try {
    ...
    if (shouldUsePeriodicNotesSettings("daily")) {
      ...
      return {
        format:
          typeof daily.format === "string"
            ? daily.format
            : DEFAULT_DAILY_NOTE_FORMAT,
        folder: typeof daily.folder === "string" ? daily.folder.trim() : "",
        template:
          typeof daily.template === "string" ? daily.template.trim() : "",
      };
    }
